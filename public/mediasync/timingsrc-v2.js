  ...
  define('util/eventify', [], function () {
	...
	*/
    ...
    const concatMap = function (
      ...
      array.forEach(function (item) {
        results.push.apply(
          results,
          projectionFunctionThatReturnsArray.call(ctx, item)
        )
      }, ctx)
      return results
    }
    ...
    const inherit = function (Child, Parent) {
      const F = function () {} // empty object to break prototype chain - hinder child prototype changes to affect parent
    ...
    function areEqual(a, b) {
      ...
      if (Array.isArray(a))
        throw new Error('illegal parameter a, array not supported', a)
      if (Array.isArray(b))
        throw new Error('illegal parameter b, array not supported', b)
	...
	*/
    ...
    HandlerMap.prototype._getID = function (handler) {
      let item
      const res = Object.keys(this._map).filter(function (id) {
        item = this._map[id]
	...
	*/
    ...
    const eventifyInstance = function (object) {
      /*
    ...
    const eventifyPrototype = function (_prototype) {
      /*
			DEFINE EVENT TYPE
      ...
      _prototype.eventifyDefineEvent = function (type, options) {
        if (type === 'events')
          throw new Error("Illegal event type : 'events' is protected")
        options = options || {}
      ...
      _prototype._eventifyMakeEItemList = function (type) {
        const makeInitEvents =
          this.eventifyMakeInitEvents ||
          function (type) {
            return []
          }
        return makeInitEvents.call(this, type).map(function (e) {
      ...
      _prototype._eventifyMakeInitEvents = function (type) {
        if (type !== 'events') {
          ...
          // type === 'events'
          const typeList = Object.keys(this._callbacks).filter(function (key) {
            return (
              key !== 'events' && this._callbacks[key]._options.init === true
            )
          }, this)
          return concatMap(
            typeList,
      ...
      _prototype._eventifyCallbackFormatter = function (type, e, eInfo) {
        const callbackFormatter =
          this.eventifyCallbackFormatter ||
          function (type, e, eInfo) {
            return [e]
          }
      ...
      _prototype.eventifyTriggerEvents = function (eItemList) {
        ...
        eItemList.forEach(function (eItem) {
          if (eItem.type === undefined)
            throw new Error('Illegal event type; undefined')
          if (eItem.type === 'events')
            throw new Error(
              "Illegal event type; triggering of events on protocted event type 'events'"
            )
        }, this)
      ...
      /*
		...
		*/
      _prototype._eventifyTriggerProtectedEvents = function (
        eItemList,
        handlerID
      ) {
      ...
      _prototype._eventifyTriggerRegularEvents = function (
        eItemList,
        handlerID
      ) {
      ...
      _prototype._eventifyTriggerEvent = function (type, e, handlerID) {
        ...
        const handlerMap = this._callbacks[type]
        const init = handlerMap._options.init
      ...
      _prototype.on = function (type, handler, ctx) {
        ...
        // register handler
        ctx = ctx || this
        const handlerID = handlerMap.register(handler, ctx)
        // do initial callback - if supported by source
        if (handlerMap._options.init) {
          // flag handler
          const handlerItem = handlerMap.getItem(handlerID)
          handlerItem.pending = true
          const self = this
          const immediateCallback = function () {
            const eItemList = self._eventifyMakeInitEvents(type)
      ...
      _prototype._eventifyFlushImmediateCallbacks = function () {
        if (this._eBuffer.length === 0) {
          const callbacks = this._immediateCallbacks
	...
	*/
    const BaseEventObject = function () {
	...
	*/
    ...
    EventBoolean.prototype.get = function () {
      return this._value
    }
	...
	*/
    ...
    const EventVariable = function (initValue, options) {
      options = options || {}
    ...
    EventVariable.prototype.get = function () {
      return this._value
    }

    EventVariable.prototype.set = function (newValue) {
      const eqFunc = this._options.eqFunc
    ...
    const makeEventPromise = function (ev, target) {
...
*/
  ...
  define('util/motionutils', [], function () {
    ...
    const calculateVector = function (vector, tsSec) {
      if (tsSec === undefined) {
        throw new Error('no ts provided for calculateVector')
      }
      const deltaSec = tsSec - vector.timestamp
      return {
        position:
          vector.position +
          vector.velocity * deltaSec +
          0.5 * vector.acceleration * deltaSec * deltaSec,
	...
	*/
    const getCorrectRangeState = function (vector, range) {
      const p = vector.position
      const v = vector.velocity
      const a = vector.acceleration
	...
	*/
    const checkRange = function (vector, range) {
      const state = getCorrectRangeState(vector, range)
    ...
    const calculateDirection = function (vector, tsSec) {
      /*
		...
		*/
      const freshVector = calculateVector(vector, tsSec)
      // check velocity
    ...
    const hasRealSolution = function (p, v, a, x) {
    ...
    const calculateRealSolutions = function (p, v, a, x) {
      // Constant Position
      if (a === 0.0 && v === 0.0) {
        if (p != x) return []
        else return [0.0]
    ...
    const calculatePositiveRealSolutions = function (p, v, a, x) {
    ...
    const calculateMinPositiveRealSolution = function (vector, x) {
      const p = vector.position
      const v = vector.velocity
      const a = vector.acceleration
    ...
    const calculateDelta = function (vector, range) {
      ...
      if (deltaBeforeSec !== null && deltaAfterSec !== null) {
        if (deltaBeforeSec < deltaAfterSec) return [deltaBeforeSec, range[0]]
        else return [deltaAfterSec, range[1]]
      } else if (deltaBeforeSec !== null) return [deltaBeforeSec, range[0]]
      else if (deltaAfterSec !== null) return [deltaAfterSec, range[1]]
    ...
    */
    ...
    const calculateSolutionsInInterval2 = function (vector, deltaSec, plist) {
      const solutions = []
      const p0 = vector.position
      const v0 = vector.velocity
      const a0 = vector.acceleration
      for (let i = 0; i < plist.length; i++) {
        const o = plist[i]
        if (!hasRealSolution(p0, v0, a0, o.point)) continue
        const intersects = calculateRealSolutions(p0, v0, a0, o.point)
        for (let j = 0; j < intersects.length; j++) {
          const t = intersects[j]
          if (t >= 0.0 && t <= deltaSec) {
            solutions.push([t, o])
          }
        }
    ...
    const calculateSolutionsInInterval = function (vector, deltaSec, plist) {
      ...
      for (let i = 0; i < plist.length; i++) {
        ...
        for (let j = 0; j < intersects.length; j++) {
          ...
          if (t >= 0.0 && t <= deltaSec) {
            ...
            console.log(
              'dropping event : 0<t<deltaSec is not true',
              t,
              deltaSec
            )
          }
    ...
    */
    ...
    const calculateInterval = function (vector, deltaSec) {
      const p0 = vector.position
      const v0 = vector.velocity
      const a0 = vector.acceleration
      const p1 = p0 + v0 * deltaSec + 0.5 * a0 * deltaSec * deltaSec
...
*/
  ...
  define('util/timeoututils', [], function () {
	...
	*/
    ...
    const Timeout = function (clock, callback, delay, options) {
      // clock
      this._clock = clock // seconds
      const now = this._clock.now() // seconds
      // timeout
      this._tid = null
      ...
      const time_left = this._target - this._clock.now() // seconds
      if (time_left > 10) {
        ...
        this._tid = setTimeout(function () {
          self._ontimeout()
        }, (time_left - self._options.early) * 1000)
      }
    }
    ...
    Timeout.prototype._ontimeout = function () {
      if (this._tid !== null) {
        ...
        if (time_left <= 0) {
          ...
          this._tid = setTimeout(function () {
            self._ontimeout()
          }, (time_left - this._options.early) * 1000)
        } else {
    ...
    Timeout.prototype.handleEvent = function (event) {
      if (
        event.source === window &&
        event.data.indexOf &&
        event.data.indexOf('smalldelaymsg_') === 0
      ) {
        event.stopPropagation()
        // ignore if timeout has been canceled
        const the_tid = parseInt(event.data.split('_')[1])
...
*/
  ...
  define('util/masterclock', [
    './eventify',
    './timeoututils',
  ], function (eventify, timeoututils) {
    'use strict'
    ...
    const MasterClock = function (options) {
	...
	*/
    MasterClock.prototype.adjust = function (options) {
      options = options || {}
      const now = localClock.now()
      const nowVector = this.query(now)
      if (options.skew === undefined && options.rate === undefined) {
        return
      }
      this._vector = {
        position:
          options.skew !== undefined ? now + options.skew : nowVector.position,
        velocity:
          options.rate !== undefined ? options.rate : nowVector.velocity,
...
*/
  ...
  define('timingobject/timingobject', [
    ...
    '../util/motionutils',
    '../util/masterclock',
  ], function (eventify, motionutils, MasterClock) {
    'use strict'
	...
	*/
    ...
    const TimingBase = function (options) {
      this._options = options || {}
	...
	*/
    ...
    TimingBase.prototype.query = function () {
      ...
      const vector = motionutils.calculateVector(this._vector, this.clock.now())
      const state = motionutils.getCorrectRangeState(vector, this._range)
    ...
    TimingBase.prototype.checkUpdateVector = function (vector) {
      ...
      let v = 0
      let a = 0
      const now = vector.timestamp || this.clock.now()
      if (this.isReady()) {
	...
	*/
    TimingBase.prototype._postProcess = function (vector) {
      ...
      const moving = vector.velocity !== 0.0 || vector.acceleration !== 0.0
	...
	*/
    TimingBase.prototype._renewTimeout = function () {
      if (this._options.timeout === true) {
        this._clearTimeout()
        const vector = this._calculateTimeoutVector()
        if (vector === null) {
          return
        }
        const now = this.clock.now()
        const secDelay = vector.timestamp - now
        const self = this
        this._timeout = this.clock.setTimeout(
          function () {
            self._process(self.onTimeout(vector))
          },
          secDelay,
          { anchor: now, early: 0.005 }
        )
      }
    }
	...
	*/
    TimingBase.prototype._calculateTimeoutVector = function () {
      const freshVector = this.query()
      const res = motionutils.calculateDelta(freshVector, this._range)
      const deltaSec = res[0]
      if (deltaSec === null) return null
      if (deltaSec === Infinity) return null
      const position = res[1]
      const vector = motionutils.calculateVector(
        freshVector,
        freshVector.timestamp + deltaSec
      )
	...
	*/
    ...
    const InternalProvider = function (options) {
      options = options || {}
      ...
      // vector
      const vector = this._options.vector || {
        position: 0,
        velocity: 0,
        acceleration: 0,
      }
    ...
    InternalProvider.prototype.update = function (vector) {
      const newVector = this.checkUpdateVector(vector)
      const event = new eventify.EventBoolean()
	...
	*/
    ...
    const ExternalProvider = function (provider, options) {
      options = options || {}
      options.timeout = true
      ...
      var self = this
      this._provider.on('vectorchange', function () {
        self._onVectorChange()
      })
      this._provider.on('skewchange', function () {
        self._onSkewChange()
      })
    ...
    ExternalProvider.prototype._onSkewChange = function () {
      if (!this._clock) {
        ...
        const current_skew = this._provider_clock.now() - this._clock.now()
        // skew delta = new_skew - current_skew
        const skew_delta = this._provider.skew - current_skew
	...
	*/
    ...
    ExternalProvider.prototype.onVectorChange = function (provider_vector) {
      ...
      const local_ts = provider_vector.timestamp - this._provider.skew
      return {
	...
	*/
    ...
    const TimingObjectBase = function (timingsrc, options) {
      TimingBase.call(this, options)
      this._version = 4
      /*
      ...
      this._internalOnChange = function () {
        const vector = self._timingsrc.vector
	...
	*/
      ...
      set: function (timingsrc) {
        ...
        if (!timingsrc) {
          ...
          try {
            ...
            throw new Error(
              'illegal timingsrc - not instance of timing object base and not timing provider'
            )
          }
        }
    ...
    /*
		Timing Object
	*/
    const TimingObject = function (options) {
      options = options || {}
      const timingsrc = options.timingsrc || options.provider
...
*/
  ...
  define('timingobject/skewconverter', [
    './timingobject',
  ], function (timingobject) {
    'use strict'

    const TimingObjectBase = timingobject.TimingObjectBase
    const inherit = TimingObjectBase.inherit
    ...
    Object.defineProperty(SkewConverter.prototype, 'skew', {
      ...
      set: function (skew) {
        ...
        const src_vector = this.timingsrc.vector
...
*/
  ...
  define('timingobject/delayconverter', [
    './timingobject',
  ], function (timingobject) {
    'use strict'
    ...
    var DelayConverter = function (timingObject, delay) {
      ...
      if (delay < 0) {
        throw new Error('negative delay not supported')
      }
      if (delay === 0) {
        throw new Error('zero delay makes delayconverter pointless')
      }
    ...
    DelayConverter.prototype.onVectorChange = function (vector) {
      ...
      const age = this.clock.now() - vector.timestamp
      ...
      if (age < this._delay) {
        ...
        const self = this
        const delayMillis = (this._delay - age) * 1000
        setTimeout(function () {
...
*/
  ...
  define('timingobject/scaleconverter', [
    './timingobject',
  ], function (timingobject) {
    'use strict'
    ...
    ScaleConverter.prototype.update = function (vector) {
      if (vector.position !== undefined && vector.position !== null)
        vector.position = vector.position / this._factor
      if (vector.velocity !== undefined && vector.velocity !== null)
        vector.velocity = vector.velocity / this._factor
      if (vector.acceleration !== undefined && vector.acceleration !== null)
        vector.acceleration = vector.acceleration / this._factor
      return this.timingsrc.update(vector)
    }
...
*/
  ...
  define('timingobject/loopconverter', [
    '../util/motionutils',
    './timingobject',
  ], function (motionutils, timingobject) {
    'use strict'
    ...
    /*
		...
		x: float point value
	*/
    const SegmentCoords = function (skew, length) {
      this.skew = skew
      this.length = length
    }
	...
	*/
    ...
    SegmentCoords.prototype.transformFloat = function (x, y) {
      ...
      const xPoint = this.getPoint(x)
      const yPoint = this.getPoint(y)
	...
	*/
    ...
    LoopConverter.prototype._inverse = function (y) {
      const current_y = this.query().position
      const current_x = this.timingsrc.query().position
      const diff = y - current_y
      const x = diff + current_x
    ...
    LoopConverter.prototype.query = function () {
      ...
      const vector = motionutils.calculateVector(this.vector, this.clock.now())
    ...
    LoopConverter.prototype._calculateTimeoutVector = function () {
      const freshVector = this.query()
      const res = motionutils.calculateDelta(freshVector, this.range)
      const deltaSec = res[0]
      if (deltaSec === null) return null
      const position = res[1]
      const vector = motionutils.calculateVector(
    ...
    LoopConverter.prototype._calculateInitialVector = function () {
      // parent snapshot
      const parentVector = this.timingsrc.query()
      // find correct position for looper
      const position = this._transform(parentVector.position)
      // find looper vector
      return {
        position: position,
...
*/
  ...
  define('timingobject/rangeconverter', [
    '../util/motionutils',
    './timingobject',
  ], function (motionutils, timingobject) {
    'use strict'
    ...
    const state = function () {
      let _state = RangeState.INIT
      let _range = null
      const is_special_state_change = function (old_state, new_state) {
        // only state changes between INSIDE and OUTSIDE* are special state changes.
        if (
          old_state === RangeState.OUTSIDE_HIGH &&
          new_state === RangeState.OUTSIDE_LOW
        )
          return false
        if (
          old_state === RangeState.OUTSIDE_LOW &&
          new_state === RangeState.OUTSIDE_HIGH
        )
          return false
      ...
      const set = function (new_state, new_range) {
        let absolute = false // absolute change
        let special = false // special change
	...
	*/
    ...
    RangeConverter.prototype._calculateTimeoutVector = function () {
      const freshVector = this._timingsrc.query()
      const res = motionutils.calculateDelta(freshVector, this.range)
      const deltaSec = res[0]
      if (deltaSec === null) return null
      const position = res[1]
    ...
    RangeConverter.prototype.onVectorChange = function (vector) {
      ...
      const new_state = motionutils.getCorrectRangeState(vector, this._range)
      // console.log("onVectorChange state", new_state);
      const state_changed = this._state.set(new_state, this._range)
...
*/
  ...
  define('timingobject/timeshiftconverter', [
    '../util/motionutils',
    './timingobject',
  ], function (motionutils, timingobject) {
    'use strict'
    ...
    TimeShiftConverter.prototype.onVectorChange = function (vector) {
      ...
      const newVector = motionutils.calculateVector(
        vector,
        vector.timestamp + this._timeOffset
      )
...
*/
  ...
  define('timingobject/derivativeconverter', [
    './timingobject',
  ], function (timingobject) {
    'use strict'
...
*/
  ...
  define('util/interval', [], function () {
	...
	*/
    ...
    const IntervalError = function (message) {
    ...
    var Interval = function (low, high, lowInclude, highInclude) {
      ...
      const lowIsNumber = isNumber(low)
      const highIsNumber = isNumber(high)
      ...
      if (typeof lowInclude !== 'boolean')
        throw new IntervalError('lowInclude not boolean')
      if (typeof highInclude !== 'boolean')
        throw new IntervalError('highInclude not boolean')
      this.low = low
      this.high = high
    ...
    Interval.prototype.toString = function () {
      const lowBracket = this.lowInclude ? '[' : '<'
      const highBracket = this.highInclude ? ']' : '>'
      const low = this.low === -Infinity ? '<--' : this.low // .toFixed(2);
      const high = this.high === Infinity ? '-->' : this.high // .toFixed(2);
      if (this.singular) return lowBracket + low + highBracket
    ...
    Interval.prototype.overlapsInterval = function (other) {
      if (other instanceof Interval === false)
        throw new IntervalError('paramenter not instance of Interval')
      // singularities
      if (this.singular && other.singular) return this.low === other.low
      if (this.singular) return other.coversPoint(this.low)
      if (other.singular) return this.coversPoint(other.low)
      // not overlap right
    ...
    Interval.prototype.coversInterval = function (other) {
      ...
      if (
        this.low === other.low &&
        this.lowInclude === false &&
        other.lowInclude === true
      )
        return false
      if (
        this.high === other.high &&
        this.highInclude === false &&
        other.highInclude === true
      )
        return false
      return true
    }
...
*/
  ...
  define('sequencing/sortedarraybinary', [
    '../util/interval',
  ], function (Interval) {
    'use strict'
    ...
    const SortedArrayError = function (message) {
    ...
    */
    ...
    const SortedArrayBinary = function () {
      /*
    ...
    SortedArrayBinary.prototype.insert = function (element) {
      const index = this.binaryIndexOf(element)
    ...
    */
    ...
    SortedArrayBinary.prototype.get = function (i) {
      return this.array[i]
    }
    SortedArrayBinary.prototype.list = function () {
      return this.array
    }
...
*/
  ...
  define('sequencing/axis', [
    ...
    const AxisError = function (message) {
      this.name = 'AxisError'
      this.message = message || ''
    }
	...
	*/
    ...
    Axis.prototype._insert = function (key, interval, data) {
      const res = { key: key, interval: interval, data: data }
      if (this._map.hasOwnProperty(key)) {
        // UPDATE
    ...
    Axis.prototype.updateAll = function (args) {
      ...
      args.forEach(function (arg) {
        ...
        if (typeof key !== 'string')
          throw new AxisError('key is ' + typeof key + ' - must be string')
        if (interval === undefined) {
          e = this._remove(key)
        } else {
          if (interval instanceof Interval === false)
            throw new AxisError('parameter must be instanceof Interval')
	...
	*/
    Axis.prototype.lookupKeysByInterval = function (interval) {
      ...
      const leftInterval = new Interval(-Infinity, interval.low)
      const rightInterval = new Interval(interval.high, Infinity)
      this._index.lookup(leftInterval).forEach(function (point) {
        this._reverse.getItemsByKey(point).forEach(function (item) {
          const _interval = this._map[item.value]
	...
	*/
    ...
    Axis.prototype.items = function () {
      return this.lookupByPoint()
    }
...
*/
  ...
  define('sequencing/sequencer', [
    ...
    const isMoving = function (vector) {
    ...
    var VerbType = Object.freeze({
      ...
      fromInteger: function (i) {
        ...
        throw new SequencerError(
          'illegal integer value for direction type ' + i
        )
      },
    })
    ...
    var DirectionType = Object.freeze({
      ...
      toInteger: function (s) {
        ...
        throw new SequencerError(
          'illegal string value direction type ' + string
        )
      },
      fromInteger: function (i) {
        ...
        throw new SequencerError(
          'illegal integer value for direction type' + i + ' ' + typeof i
        )
      },
    })

    // OPTYPES - defined in axis
    const OpType = axis.OpType
    ...
    const Schedule = function (now, options) {
      this.queue = []
      // options
      ...
      // time-interval
      const start = now
      const end = now + this.options.lookahead
    ...
    Schedule.prototype.getTimeInterval = function () {
      return this.timeInterval
    }
    Schedule.prototype.getPosInterval = function () {
      return this.posInterval
    }
    Schedule.prototype.setPosInterval = function (interval) {
      this.posInterval = interval
    }
    Schedule.prototype.sortFunc = function (a, b) {
      return a.ts - b.ts
    }
    ...
    Schedule.prototype.push = function (now, ts, task) {
      if (this.timeInterval.coversPoint(ts)) {
        ...
        if (ts >= now) {
          ...
          console.log(
            'Schedule : task pushed a bit too late, ts < now ',
            ts - now
          )
        }
      }
	...
	*/
    ...
    Schedule.prototype.advance = function (now) {
      if (now < this.timeInterval.low) {
        console.log(
          'Schedule : Advancing backwards ' + (now - this.timeInterval.low)
        )
      }
      const start = now
      const end = now + this.options.lookahead
	...
	*/
    ...
    const SequencerEArgs = function (
      ...
      const pointType = sequencer._axis.getPointType(point, interval)
      this.src = sequencer
      this.key = key
    ...
    /*
		SequencerCue
	*/
    const SequencerCue = function (item) {
      this.key = item.key
    ...
    SequencerCue.prototype.toString = function () {
      let s = this.key + ' ' + this.interval.toString()
      if (this.data) s += ' ' + JSON.stringify(this.data)
      return s
    }
	...
	*/
    var Sequencer = function (timingObject, _axis) {
      ...
      this._wrappedOnTimingChange = function () {
        this._onTimingChange()
      }
      this._wrappedOnAxisChange = function (eItemList) {
        const eArgList = eItemList.map(function (eItem) {
          return eItem.e
        })
	...
	*/
    ...
    Sequencer.prototype._onTimingChange = function (event) {
      ...
      /*
	    ...
	    */
      const nowVector = motionutils.calculateVector(initVector, now)

      const newKeys = this._axis.lookupKeysByPoint(nowVector.position)

      // exitKeys are in activeKeys - but not in newKeys
      const exitKeys = Object.keys(this._activeKeys).filter(function (key) {
        return !newKeys.hasOwnProperty(key)
      })
      // enterKeys are in newKeys - but not in activeKeys
      const enterKeys = Object.keys(newKeys).filter(function (key) {
      ...
      /*
	    ...
	    */
      const _isMoving = isMoving(initVector)
      if (_isMoving) {
        const nowPos = nowVector.position
        const points = this._axis.lookupByInterval(
          new Interval(nowPos, nowPos, true, true)
        )
        points.forEach(function (pointInfo) {
          ...
          if (pointInfo.pointType === axis.PointType.SINGULAR) {
            ...
            // closed interval?
            const interval = pointInfo.interval
            let closed = false
            if (
              pointInfo.pointType === axis.PointType.LOW &&
              interval.lowInclude
            ) {
              closed = true
            } else if (
              pointInfo.pointType === axis.PointType.HIGH &&
              interval.highInclude
            ) {
              closed = true
            }
            // exiting or entering interval?
            const direction = DirectionType.fromInteger(
              motionutils.calculateDirection(initVector, now)
            )
            let entering = true
            if (
              pointInfo.pointType === axis.PointType.LOW &&
            ...
            if (
              pointInfo.pointType === axis.PointType.HIGH &&
              direction === DirectionType.FORWARDS
            )
              entering = false
      ...
      const exitItems = exitKeys.map(function (key) {
        return this._axis.getItem(key)
      }, this)
      const enterItems = enterKeys.map(function (key) {
        return this._axis.getItem(key)
      }, this)
      // Trigger interval events

      const eList = this._processIntervalEvents(now, exitItems, enterItems, [])
    ...
    const removeDuplicates = function (opList) {
      if (opList.length < 2) {
        return opList
      }
      const res = []
      let op
      const map = {} // op.key -> opList index
    ...
    Sequencer.prototype._onAxisChange = function (origOpList) {
      ...
      const enterItems = [] // {key:key, interval:interval}
      const exitItems = [] // {key:key, interval:interval}
      let isActive, shouldBeActive

      // filter out REPEATs to get only operations where interval changed
      const opList = origOpList.filter(function (op) {
        return op.type !== OpType.REPEAT
      })

      let i, e, key, interval, data, opType
      ...
      /*
		...
		*/
      const exitKeys = exitItems.map(function (item) {
        return item.key
      })
      const changeItems = origOpList
        .filter(function (op) {
      ...
      /*
		...
		*/
      const _isMoving = isMoving(nowVector)
      if (!_isMoving) {
        var eList = self._processIntervalEvents(
          now,
          exitItems,
          enterItems,
          changeItems
        ...
        opList.forEach(function (op) {
          ...
          if (_isMoving) {
            /*
				    ...
				    */
            const item = { key: key, interval: interval, data: data }
            const rangeInterval = this._schedule.getPosInterval()
            if (rangeInterval !== null) {
              ...
              item.pointType = this._axis.getPointType(
                item.point,
                item.interval
              )
	...
	*/
    Sequencer.prototype._main = function (now, isTimeout) {
      ...
      const _isMoving = isMoving(this._to.vector)
      ...
      if (_isMoving) {
        ...
        if (newTimeoutRequired) {
          ...
          this._clearTimeout()
          // update timeout
          const secAnchor = this._to.clock.now()
          const secDelay = this._schedule.getDelayNextTs(secAnchor) // seconds
          this._currentTimeoutPoint = nextTimeoutPoint
          const self = this
          // console.log("main done - set timeout", this._schedule.queue.length);
          this._timeout = this._to.clock.setTimeout(
            function () {
              self._clearTimeout()
              self._main(undefined, true)
            },
            secDelay,
            { anchor: secAnchor, early: 0.005 }
          )
        }
      }
    }
	...
	*/
    ...
    Sequencer.prototype._load = function (now, givenPoints) {
      ...
      /*
	    ...
	    */
      const timeInterval = this._schedule.getTimeInterval()
      const tStart = timeInterval.low
      const tEnd = timeInterval.high
      const tDelta = timeInterval.length
      // range
      const range = this._to.range
      const vectorStart = motionutils.calculateVector(initVector, tStart)
      ...
      if (!points) {
        ...
        const posRange = motionutils.calculateInterval(vectorStart, tDelta)
        const pStart = Math.round10(Math.max(posRange[0], range[0]), -10)
        const pEnd = Math.round10(Math.min(posRange[1], range[1]), -10)
      ...
      const eventList = motionutils.calculateSolutionsInInterval(
        vectorStart,
        tDelta,
        points
      )
      ...
      if (points.length !== eventList.length) {
        console.log(
          'warning : mismatch points and events',
          points.length,
          eventList.length
        )
      }
      ...
      /*
	    ...
	    */
      const rDelta = motionutils.calculateDelta(vectorStart, range)[0]
      ...
      eventList.forEach(function (e) {
        ...
        /*
			...
			*/
        if (
          task.pointType === axis.PointType.LOW ||
          task.pointType === axis.PointType.HIGH
        ) {
          const v = motionutils.calculateVector(initVector, tStart + d)
          if (v.velocity === 0) {
    ...
    Sequencer.prototype._processScheduleEvents = function (now, eventList) {
      ...
      eventList.forEach(function (e) {
        if (e.task.interval.singular) {
          ...
          const directionType = DirectionType.fromInteger(directionInt)
          const pointType = this._axis.getPointType(
            e.task.point,
            e.task.interval
          )
          const pointInt = axis.PointType.toInteger(pointType)
    ...
    Sequencer.prototype._processIntervalEvents = function (
      now,
      exitItems,
      enterItems,
      changeItems
	...
	*/
      ...
      eArgList
        ...
        .forEach(function (eArg) {
          ...
          if (eArg.pointType === axis.PointType.SINGULAR) {
            ...
            let closed = false
            if (
              eArg.pointType === axis.PointType.LOW &&
              eArg.interval.lowInclude
            ) {
              closed = true
            } else if (
              eArg.pointType === axis.PointType.HIGH &&
              eArg.interval.highInclude
            ) {
              closed = true
            }
            if (eArg.type === VerbType.ENTER) {
    ...
    Sequencer.prototype.removeCue = function (key, removedData) {
      return this.updateAll([
        { key: key, interval: undefined, data: removedData },
      ])
    }
...
*/
  ...
  define('sequencing/windowsequencer', [
    '../util/eventify',
    '../util/motionutils',
    './axis',
    './sequencer',
    ...
    const Sequencer = seq.DefaultSequencer
	...
	*/
    WindowSequencer.prototype.eventifyMakeInitEvents = function (type) {
      if (type === 'change') {
        ...
        return Object.keys(this._activeKeys).map(function (key) {
          const item = this._axis.getItem(key)
          return {
            key: key,
	...
	*/
    WindowSequencer.prototype._getActiveInterval = function () {
      const vectorA = this._toA.query()
      const vectorB = this._toB.query()
      const start = Math.min(vectorA.position, vectorB.position)
      const end = Math.max(vectorA.position, vectorB.position)
	...
	*/
    WindowSequencer.prototype._getOpFromAxisOpList = function (
      axisOpList,
      key
    ) {
      let op
      if (axisOpList) {
        for (let i = 0; i < axisOpList.length; i++) {
          const item = axisOpList[i]
          if (item.key === key) {
            op = item
            break
          }
	...
	*/
    ...
    WindowSequencer.prototype._reevaluate = function (axisOpList) {
      ...
      const newKeys = this._axis.lookupKeysByInterval(activeInterval)
      ...
      if (axisOpList) {
        axisOpList.forEach(function (op) {
          if (
            this._activeKeys.hasOwnProperty(op.key) &&
            newKeys.hasOwnProperty(op.key)
          ) {
	...
	*/
    ...
    // return all cues covered by searchInterval
    WindowSequencer.prototype.getCuesCoveredByInterval = function (
      searchInterval
    ) {
...
*/
  ...
  define('sequencing/timingcallbacks', [
    '../util/motionutils',
  ], function (motionutils) {
    'use strict'
    ...
    const TimingCallbackBase = function (timingObject, handler) {
      this._timingsrc = timingObject
      this._handler = handler
      this._timeout = null
      this._wrappedOnChange = function () {
        this._onChange()
      }
      // initialise
    ...
    TimingCallbackBase.prototype._renewTimeout = function () {
      this._clearTimeout()
      const res = this._calculateTimeout()
      if (res.delay === null) return null
      const self = this
      this._timeout = this._timingsrc.clock.setTimeout(
        function () {
          self._onTimeout()
        },
        res.delay,
        { anchor: res.anchor, early: 0.005 }
      )
    }
  ...
  */
    ...
    var SetPointCallback = function (timingObject, handler, point, options) {
      ...
      this._options.repeat =
        this._options.repeat !== undefined ? this._options.repeat : false
      this._point = point
    }
    ...
    SetPointCallback.prototype._calculateTimeout = function () {
      ...
      const delay = motionutils.calculateMinPositiveRealSolution(
        vector,
        this._point
      )
      return {
  ...
  */
    ...
    var SetIntervalCallback = function (
      ...
      this._options.offset =
        this._options.offset !== undefined ? this._options.offset : 0
      this._length = length
    }
    ...
    SetIntervalCallback.prototype._getPoint = function (x) {
      const skew = this._options.offset
      return {
    ...
    SetIntervalCallback.prototype._onChange = function () {
      const points = this._calculatePoints(this._timingsrc.query().position)
      if (points.isTarget) {
        this._handler()
      }
      this._renewTimeout()
  ...
  */
    ...
    SetIntervalCallback.prototype._calculateTimeout = function () {
      const vector = this._timingsrc.query()
      const points = this._calculatePoints(vector.position)
      const delay = motionutils.calculateDelta(vector, [
        points.before,
        points.after,
      ])[0]
      return {
    ...
    // module definition
    return {
      setPointCallback: function (timingObject, handler, point, options) {
        return new SetPointCallback(timingObject, handler, point, options)
      },
      setIntervalCallback: function (timingObject, handler, length, options) {
        return new SetIntervalCallback(timingObject, handler, length, options)
      },
    }
  })
  /*
...
*/
  ...
  define('sequencing/timinginteger', [
    '../util/motionutils',
    '../util/eventify',
  ], function (motionutils, eventify) {
    /*
  ...
  */
    ...
    const TimingInteger = function (timingObject, options) {
      // timingsrc
      this._timingsrc
  ...
  */
    ...
    TimingInteger.prototype._refresh = function () {
      let value = Math.floor(this._timingsrc.query().position)
  ...
  */
    ...
    TimingInteger.prototype._calculatePoints = function (position) {
      let before, after
      const isTarget = Number.isInteger(position)
      if (isTarget === true) {
        before = position - 1
        after = position + 1
      } else {
  ...
  define('sequencing/activecue', ['../util/eventify'], function (eventify) {
	...
	*/
    ...
    const ActiveCue = function (seq) {
      ...
      const self = this
      this._wrappedHandler = function (eList) {
        self._onSeqEvents(eList)
      }
    ...
    ActiveCue.prototype._refresh = function () {
      ...
      // pick last item
      const value = len > 0 ? this._stack[len - 1][1] : undefined
      if (value !== this._value) {
        this._value = value
    ...
    ActiveCue.prototype._onSeqEvents = function (eList) {
      eList.forEach(function (eArg) {
        ...
        if (eArg.type === 'change') {
          ...
          const i = this._stack.findIndex(function (element, index, array) {
    ...
    ActiveCue.prototype.get = function () {
      return this._value
    }
...
*/
  ...
  define('mediasync/mediasync', [], function () {
    ...
    function needKick(elem) {
      throw new Error(
        'MediaSync is no longer maintained as part of Timingsrc. Get MediaSync from https://mcorp.no/lib/mediasync.js'
      )
    }
    ...
    const MediaSync = function (elem, timingObject, options) {
      throw new Error(
        'MediaSync is no longer maintained as part of Timingsrc. Get MediaSync from https://mcorp.no/lib/mediasync.js'
      )
    }
